...
`sysprompts/hidream.txt`, creating a single flowing paragraph, but
...
The release file includes four fields (so far): prompt, uuid, json,
and tags, where the prompt field is a flattened "paragraph" version of
each prompt auto-generated by `scripts/json2txt.py` and then run
through the
[z-image-engineer-v6](https://huggingface.co/BennyDaBall/Z-Image-Engineer-V6)
LLM with Juan's
[`sysprompts/compress.txt`](https://github.com/jtreminio/word-salad/blob/main/llm_prompts/compress.txt)
system prompt.

The classification tags included in the release are generated with
`sysprompts/tagging.txt` using the `qwen3-vl-4b` model, and then
passed through `scripts/clean_tags.py` for consistency.

## JSON prompt schema

The basic structure of a processed prompt is shown below. The five
...
- `scripts/make_release.py` - create a gzipped JSONL release file
  containing LLM-enhanced prompts alongside the original JSON.

- `scripts/tagging.sh` - use LM Studio's `lms` CLI to convert LLM
   prompts to classification tags, using `sysprompts/tagging.txt`

- `scripts/clean_tags.py` - clean up LLM-generated tags to add to
  the releases. If a file `ranked-tags.txt` is present, it is used
  to order known tags and truncate the list so that the least common
  tags are omitted.
